...
3. Move the directory:
   ...
   mv ~/playground/liquid-labs/sdlcpilot-cli ~/playground/sdlcforge/core-cli
   ```
   Adjust the exact `mv` invocation for the actual resolved playground root if it differs from `~/playground`.
4. Verify the moved checkout is still a healthy git repository (`git -C ~/playground/sdlcforge/core-cli status`, `git -C ~/playground/sdlcforge/core-cli remote -v`) and that any registered Flow plan/session state (`.flow/`) still resolves correctly from the new path — re-run `flow-mcp` project-index refresh if available, since the project index caches paths.
...
- `~/playground/sdlcforge/core-cli` exists and is a healthy git checkout (`git status`, `git log -1` both succeed).
